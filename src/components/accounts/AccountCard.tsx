    ...
    <Card className={`relative overflow-hidden ${isDragging ? "opacity-50" : ""}`}>
      {compact ? (
        <CardContent className="p-3 flex flex-col items-center justify-center aspect-square">
          {dragHandleProps && (
            <button
              className="absolute top-1.5 left-1.5 cursor-grab active:cursor-grabbing text-muted-foreground hover:text-foreground touch-none"
              {...dragHandleProps}
            >
          ...
          <div
            className="flex items-center justify-center rounded-full transition-transform duration-200 hover:scale-110 size-8"
            style={{ backgroundColor: account.color + "20" }}
          >
            <Icon className="size-4" style={{ color: account.color }} />
          </div>
          <p className="font-medium text-xs mt-2 text-center truncate w-full">{account.name}</p>
          <p className="text-lg font-display font-black tabular-nums mt-1">
